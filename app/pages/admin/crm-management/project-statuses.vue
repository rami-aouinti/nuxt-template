...
import type { CrmProjectStatus } from '~/types/crm'
...
const mapItem = (item: CrmProjectStatus) => ({
  id: item.id,
  name: item.name,
  createdAt: item.createdAt,
  updatedAt: item.updatedAt,
  actions: buildCrmAdminResourceActionLinks(
    item['@id'] ?? `/project_statuses/${item.id}`,
  ),
})
</script>
...
<template>
  <CrmResourcePage
    :title="t('navigation.crmProjectStatuses')"
    endpoint="/project_statuses"
    :headers="headers"
    :map-item="mapItem"
    :search-fields="['name']"
  />
