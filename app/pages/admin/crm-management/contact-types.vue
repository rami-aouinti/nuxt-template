...
await contactTypeCollection.fetch()

const contactTypes = computed(() => contactTypeCollection.data.value?.member ?? [])
...
async function handleSubmit() {
  ...
  try {
    ...
    resetForm()
    await contactTypeCollection.refresh()
  } catch (error) {
    console.error(error)
    Notify.error("Impossible d'enregistrer le type de contact")
  } finally {
    loading.value = false
  }
}
...
async function handleDelete(id: number) {
  ...
  try {
    await $fetch(`/api/contact_types/${id}`, {
      method: 'DELETE',
      headers: requestHeaders,
      credentials: 'include',
    })
  ...
  <v-container fluid>
    <v-row>
      <v-col cols="12" md="4">
        <v-card>
          <v-card-title>
            {{ editing ? 'Modifier un type de contact' : 'Ajouter un type de contact' }}
          </v-card-title>
          <v-card-text>
      ...
      <v-col cols="12" md="8">
        <v-card>
          <v-card-title>Types de contact</v-card-title>
          <v-data-table
            :items="contactTypes"
            :loading="contactTypeCollection.pending.value"
            :headers="[
