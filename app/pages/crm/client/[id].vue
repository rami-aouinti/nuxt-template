...
const pageTitle = computed(() => {
  ...
  const label =
    name && name.length ? name : clientId.value ? `#${clientId.value}` : null
    ...
    <client-only>
      <teleport to="#app-drawer">
        <AppNavigationList
          ...
          :title="
            translate('crm.client.drawer.projectsTitle', 'Projets du client')
          "
          :description="
            translate(
          ...
          <v-list v-if="contactItems.length" density="comfortable">
            <v-list-item v-for="contact in contactItems" :key="contact.id">
              ...
              <v-list-item-subtitle class="text-body-2 text-medium-emphasis">
                {{
                  translate(
                    'crm.client.drawer.contactLabel',
                    'Coordonnée enregistrée',
                  )
                }}
            ...
            >
              <div class="d-flex flex-column gap-2">
                <div class="text-h5 font-weight-bold">
                  {{
                    client?.name ||
                    translate('crm.client.header.placeholder', 'Client CRM')
                  }}
                </div>
                ...
                <div class="d-flex flex-wrap gap-2">
                  <v-chip color="primary" variant="tonal" size="small">
                    {{ translate('crm.client.header.projects', 'Projets') }}:
                    {{ clientProjects.length }}
                  </v-chip>
                  <v-chip color="secondary" variant="tonal" size="small">
                    {{ translate('crm.client.header.documents', 'Documents') }}:
              ...
              <div class="d-flex flex-column gap-2 align-end">
                ...
                <v-chip color="secondary" variant="tonal" size="small">
                  {{ translate('crm.client.header.contacts', 'Contacts') }}:
                  {{ contactItems.length }}
                </v-chip>
              </div>
            </div>
        ...
        <v-col cols="12" md="6">
          <AppCard class="pa-4" elevation="1">
            ...
            <div v-else class="text-body-2 text-medium-emphasis">
              {{
                translate(
                  'crm.client.projects.empty',
                  'Aucun projet lié pour le moment.',
                )
              }}
            </div>
          </AppCard>
        ...
        <v-col cols="12" md="6">
          <AppCard class="pa-4" elevation="1">
            ...
            <v-list v-if="clientDocuments.length" density="comfortable">
              <v-list-item
                v-for="document in clientDocuments"
                :key="document.id"
              >
                <v-list-item-title class="font-weight-medium">{{
                  document.name
