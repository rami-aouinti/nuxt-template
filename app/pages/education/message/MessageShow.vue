<template>
  <div v-if="item">
    ...
    <div v-if="myReceiver" class="message-show__tags-container">
    ...
    <div class="field space-x-4">
      <span>{{ t('From') }}</span>
      <BaseAvatarList v-if="item.sender" :users="[item.sender]" />
      <span v-else v-t="'No sender'" />
    </div>
    ...
    <template v-if="item.attachments && item.attachments.length > 0">
      <BaseCard>
        ...
        <ul class="space-y-2">
          <li v-for="(attachment, index) in item.attachments" :key="index">
            <audio
            ...
            <a v-else :href="attachment.downloadUrl" class="btn btn--plain">
...
import BaseChip from '../../../components/education/basecomponents/BaseChip.vue'
import BaseAutocomplete from '../../../components/education/basecomponents/BaseAutocomplete.vue'
...
import BaseCard from '../../../components/education/basecomponents/BaseCard.vue'
...
const receiverType = route.query.receiverType
  ? parseInt(route.query.receiverType)
  : MESSAGE_TYPE_INBOX
store.dispatch('message/load', id).then((responseItem) => {
  item.value = responseItem

  const rawRt = route.query.receiverType
    ? Number(route.query.receiverType)
    : undefined
  ...
  if (!myReceiver.value) {
    const all = [
      ...
      ...responseItem.receiversSender,
    ]
    myReceiver.value = all.find(
...
function getTagIndex(tag) {
  return myReceiver.value.tags.findIndex(
    (receiverTag) => receiverTag['@id'] === tag['@id'],
  )
}
...
async function onSearchTags(query) {
  ...
  const { items } = await messageTagService.searchUserTags(
