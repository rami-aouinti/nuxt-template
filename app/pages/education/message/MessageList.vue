          ...
          >
            <InputGroup>
              <InputText
                ...
                type="text"
              />
              <BaseButton icon="search" is-submit type="primary" />
              <BaseButton icon="close" type="primary" @click="onResetSearch" />
            </InputGroup>
          </form>
        </template>
          ...
          <template #body="slotProps">
            <BaseAppLink
              ...
              :class="[
                ...
                {
                  'font-bold':
                    showingInbox && !findMyReceiver(slotProps.data)?.read,
                },
              ]"
            >
        ...
        >
          <div class="flex items-center space-x-4">
            ...
            <div
              ...
              v-text="t('No sender')"
            />
            <BaseAvatarList v-else :users="mapReceiverMixToUsers(item)" />
            <div class="flex-1">
          ...
          <div class="mt-4">
            ...
            <BaseAppLink
              ...
              :class="[
                ...
                { 'font-bold': showingInbox && !findMyReceiver(item)?.read },
              ]"
            >
              {{ item.title }}
            </BaseAppLink>
          ...
          <div class="mt-2">
            ...
            <div class="text-base text-gray-500">
              {{ abbreviatedDatetime(item.sendDate) }}
            </div>
          </div>
...
import BaseTag from '../../../components/education/basecomponents/BaseTag.vue'
...
const tags = computed(
  () =>
    messageTagsResult.value?.messageTags?.edges.map(({ node }) => node) ?? [],
)
...
function onPage(event) {
  ...
  fetchPayload[`order[${event.sortField}]`] =
...
function findMyReceiver(
  message,
  receiverType = showingInbox.value ? MESSAGE_TYPE_INBOX : MESSAGE_TYPE_SENDER,
) {
  const receivers = [
    ...message.receiversTo,
    ...message.receiversCc,
...
onMounted(() => {
  onMessageRead = (ev) => {
    ...
    const mine = receivers.find(
      (r) =>
        r.receiver?.['@id'] === receiverId && r.receiverType === receiverType,
    )
    if (mine) {
      mine.read = true
    }
