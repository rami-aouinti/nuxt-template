    ...
    <BaseTable :is-loading="loading" :values="exchanges" data-key="id">
      <Column :header="t('Third party')">
        <template #body="{ data }">
          {{
            thirdParties.find(
              (tp) => `/api/third_parties/${tp.id}` === data.thirdParty,
            )?.title || '-'
          }}
        </template>
      </Column>
      ...
      <Column v-if="exchanges.some((e) => !e.allUsers)" :header="t('Users')">
        <template #body="{ data }">
          <div
...
import BaseCheckbox from '../../../components/education/basecomponents/BaseCheckbox.vue'
import BaseCalendar from '../../../components/education/basecomponents/BaseCalendar.vue'
import BaseMultiSelect from '../../../components/education/basecomponents/BaseMultiSelect.vue'
...
const dialogTitle = computed(() => (isEditing.value ? t('Edit') : t('Add')))

const selectedThirdPartyName = computed(() => {
  return (
    thirdParties.value.find((tp) => tp.id === thirdPartyId.value)?.title || null
  )
})
...
const fetchExchangeUsers = async () => {
  try {
    ...
    all.forEach((item) => {
      ...
      const fullUser = users.value.find(
        (u) => `/api/users/${u.id}` === item.user,
      )
      grouped[key].push({
...
const openEdit = (row) => {
  ...
  editingExchangeIri.value =
    row['@id'] || `/api/third_party_data_exchanges/${row.id}`
  ...
  if (!form.value.allUsers) {
    ...
    form.value.selectedUsers = assigned
      .map((u) => u.id)
      .filter((id) => id != null)
  } else {
  ...
  try {
    ...
    if (isEditing.value) {
      ...
      const currentIds = new Set(
        current.map((u) => u.id).filter((id) => id != null),
      )
      ...
      if (toRemoveIds.length > 0) {
        const toRemoveRels = current
          ...
          .filter(Boolean)
        await Promise.all(
          toRemoveRels.map((rel) => adminService.deleteExchangeUser(rel)),
        )
      }
