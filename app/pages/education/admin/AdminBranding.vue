<template>
  <div>
    ...
    <p class="mb-8 text-sm opacity-80">
      {{
        t(
          'Upload optional header and email logos. SVG is preferred; PNG is used as fallback.',
        )
      }}
    </p>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
      <div
        class="p-6 rounded-2xl shadow-sm border border-gray-200 bg-white space-y-6"
      >
        <div>
        ...
        <section class="space-y-3">
          ...
          <div class="flex items-center gap-3">
            <span
              class="inline-flex items-center rounded-full border px-2 py-0.5 text-[11px] uppercase"
              >SVG</span
            >
            <img
              :key="'h-s-' + cacheBust"
...
const headerSvgStrict = computed(() =>
  u(slug.value, 'images/header-logo.svg', { strict: true }),
)
const headerPngStrict = computed(() =>
  u(slug.value, 'images/header-logo.png', { strict: true }),
)
const emailSvgStrict = computed(() =>
  u(slug.value, 'images/email-logo.svg', { strict: true }),
)
const emailPngStrict = computed(() =>
  u(slug.value, 'images/email-logo.png', { strict: true }),
)
...
function markExists(which, value) {
  switch (which) {
    case 'headerSvg':
      headerSvgExists.value = value
      break
    case 'headerPng':
      headerPngExists.value = value
      break
    case 'emailSvg':
      emailSvgExists.value = value
      break
    case 'emailPng':
      emailPngExists.value = value
      break
  }
}
