<template>
  <div v-if="item">
    <p class="text-body-1 font-semibold" v-text="item.title" />
...
watch(item, (val) => {
  ...
  fetchLanguageNameFromApi(iso)
    .then((name) => {
      if (name) languageLabel.value = name
    })
    .catch(() => {})
})
