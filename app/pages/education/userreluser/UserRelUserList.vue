<template>
  ...
  <div class="flex flex-col gap-4">
    <div class="w-full">
      ...
      >
        <Skeleton v-for="i in 6" :key="i" height="10.5rem" />
      </div>
      <DataView v-else :value="items" class="friend-list" layout="grid">
        <template #grid="slotProps">
          <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            ...
            >
              ...
              <div v-if="isCurrentUser" class="friend-options">
                <span
...
function reloadHandler() {
  ...
  Promise.all([
    userRelUserService.findAll({
      params: { user: user.value.id, relationType: 3 },
    }),
    userRelUserService.findAll({
      params: { friend: user.value.id, relationType: 3 },
    }),
  ])
    ...
    .then(([friendshipJson, friendshipBackJson]) => {
      ...
      items.value = [
        ...friendshipJson['hydra:member'],
        ...friendshipBackJson['hydra:member'],
      ].filter((friend) => {
        const friendId =
          friend.user['@id'] === user.value['@id']
            ? friend.friend['@id']
            : friend.user['@id']
