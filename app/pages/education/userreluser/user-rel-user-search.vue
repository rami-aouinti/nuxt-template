<template>
  ...
  <div class="user-rel-user-search">
    ...
    <div v-if="!loadingResults" class="results-list">
      <div v-for="(user, index) in foundUsers" :key="index" class="user-card">
...
async function fetchFriendsList() {
  try {
    const friendshipList = await userRelUserService.getFriendList(
      securityStore.user['@id'],
    )

    friendsList.value = friendshipList
      .map((friendship) => friendship.friend.id)
      .concat(securityStore.user.id)
  } catch (error) {
...
const asyncFind = async (query) => {
  ...
  try {
    ...
    foundUsers.value = items.filter(
      (foundUser) => !friendsList.value.includes(foundUser.id),
    )
  } catch (error) {
...
const addFriend = async (friend) => {
  try {
    await userRelUserService.sendFriendRequest(
      securityStore.user['@id'],
      friend['@id'],
    )
