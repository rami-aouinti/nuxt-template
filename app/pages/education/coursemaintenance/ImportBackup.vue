<template>
  <div class="space-y-6 cm-import-backup">
    ...
    <div class="flex items-center gap-3">
      <Step :index="1" :current="step" :label="t('Select source')" />
      <Line />
      <Step :index="2" :current="step" :label="t('Options')" />
      <Line />
      <Step :index="3" :current="step" :label="t('Select items')" />
      <Line />
      <Step :index="4" :current="step" :label="t('Restore')" />
    </div>
    ...
    <section v-if="step === 1" class="grid grid-cols-1 gap-4 lg:grid-cols-2">
      <div class="rounded-lg border border-gray-25 p-4">
        ...
        <div class="space-y-3">
          ...
          >
            ...
            <option v-for="b in backups" :key="b.file" :value="b.file">
              {{ b.label }}
            </option>
          </select>
        </div>
      ...
      <div class="rounded-lg border border-gray-25 p-4">
        ...
        <div class="space-y-3">
          <label class="flex items-center gap-2">
            <input v-model="importOption" type="radio" value="full_backup" />
            <span class="text-sm text-gray-90">{{
              t('Import full backup')
            }}</span>
          </label>
          <label class="flex items-center gap-2">
            <input v-model="importOption" type="radio" value="select_items" />
            <span class="text-sm text-gray-90">{{
    ...
    <section v-if="step === 2" class="space-y-4">
      <CMInfo :title="t('Ready to import')">
        <template #body>
          <p class="text-sm text-gray-50">
            {{
              t(
                'We will restore the backup into this course. You can go back to change options.',
              )
            }}
          </p>
        </template>
      ...
      <div class="flex justify-between">
        ...
        <button class="btn-primary" :disabled="loading" @click="doRestore">
          <i class="mdi mdi-database-import-outline" /> {{ t('Start import') }}
        </button>
      </div>
    </section>
    ...
    <section v-if="step === 3" class="space-y-4">
      ...
      <ResourceSelector
        ...
        :empty-text="t('No resources available in this backup.')"
      />
      ...
      <div class="flex justify-between">
        ...
        <button class="btn-primary" :disabled="loading" @click="doRestore">
          <i class="mdi mdi-database-import-outline" />
          {{ t('Import selected') }}
        </button>
      </div>
    </section>
...
function pushWithQuery(targetPathOrName) {
  ...
  if (typeof targetPathOrName === 'string') {
    ...
    router.push({
      ...targetPathOrName,
      query: { ...route.query, ...(targetPathOrName.query || {}) },
    })
  }
}
...
async function nextFromStep1() {
  ...
  try {
    ...
    if (backupType.value === 'local') {
      if (!localFile.value)
        throw new Error(
          t('Please select a backup file (.zip, .mbz, .tgz, .gz).'),
        )
      res = await svc.uploadFile(node.value, localFile.value)
    } else {
      if (!serverFilename.value)
        throw new Error(t('Please choose a server backup.'))
    ...
    error.value =
      e?.response?.data?.error || e?.message || t('Error selecting source.')
  } finally {
    loading.value = false
  }
}
