  ...
  <div class="space-y-6">
    ...
    <section v-if="step === 1" class="grid grid-cols-1 gap-4 lg:grid-cols-2">
      <div class="rounded-lg border border-gray-25 p-4">
        ...
        <label class="mb-2 flex items-center gap-2">
          <input v-model="scope" type="radio" value="full" />
        ...
        <CMAlert
          ...
          :text="
            t(
              'Selected items will be the only ones included in the exported file.',
            )
          "
        />
      </div>
      ...
      <div class="rounded-lg border border-gray-25 p-4 space-y-3">
        <h3 class="mb-1 text-sm font-semibold text-gray-90">
          {{ t('Moodle export') }}
        </h3>

        <label class="block text-xs font-medium text-gray-60">{{
          t('Moodle version')
        }}</label>
        <select
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div class="sm:col-span-1">
            <label class="block text-xs font-medium text-gray-60 mb-1"
              >{{ t('Admin ID') }} *</label
            >
            <input
            ...
            <p class="mt-1 text-[11px] leading-4 text-gray-50">
              {{
                t(
                  'Moodle requires an internal user id stored in XML. If in doubt, write 1.',
                )
              }}
            </p>
          </div>
        ...
        <CMAlert
          ...
          :text="
            t(
              'A .mbz file will be generated. You will be able to download it once finished.',
            )
          "
        />
      </div>
      ...
      <div class="col-span-full flex justify-end gap-3">
        ...
        >
          ...
          {{ scope === 'selected' ? t('Continue') : t('Create .mbz') }}
        </button>
      </div>
    </section>
    ...
    <section v-if="step === 2" class="space-y-4">
      ...
      <div class="flex justify-between">
        ...
        <button class="btn-primary" :disabled="loading" @click="doExport">
          <i class="mdi mdi-download" /> {{ t('Create .mbz') }}
        </button>
      </div>
    </section>
      ...
      <CMInfo :title="t('Export completed')">
        <template #body>
          <p class="text-sm text-gray-50">
            {{
              t(
                'Your Moodle backup has been created. You can find it in the course backups list.',
              )
            }}
          </p>
        </template>
...
const canContinueFromStep1 = computed(() => {
  ...
  return (
    idOk && adminLogin.value.trim() !== '' && adminEmail.value.trim() !== ''
  )
})
...
async function doExport() {
  ...
  try {
    ...
    error.value =
      ...
      e?.message ||
      t('Failed to create Moodle backup.')
  } finally {
    loading.value = false
  }
}
