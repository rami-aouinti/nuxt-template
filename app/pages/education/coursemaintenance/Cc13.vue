      ...
      <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
        ...
        <div class="rounded-lg border border-gray-100 p-4">
          ...
          <div class="mb-3">
            ...
            <p v-if="scope === 'selected'" class="mt-1 text-xs text-gray-500">
          ...
          >
            <div class="flex items-center justify-between border-b px-3 py-2">
              ...
              <div class="flex gap-2 text-xs">
                <button class="link" @click="checkAll(true)">
                  {{ t('Select all') }}
                </button>
                <span class="text-gray-300">•</span>
                <button class="link" @click="checkAll(false)">
                  {{ t('Clear') }}
                </button>
              </div>
            </div>

            <div v-if="loadingResources" class="p-3 text-xs text-gray-500">
              {{ t('Loading resources…') }}
            </div>
              ...
              >
                ...
                >
                  <span class="flex items-center gap-2">
                    <i class="mdi mdi-folder-outline" />{{ group.title }}
                  </span>
                  <i
                    :class="
                      open[group.type]
                  ...
                  <template v-else>
                    ...
                    >
                      ...
                      <div
                        ...
                        v-if="
                          Array.isArray(kids(cat)) &&
                          kids(cat).some(
                            (n) => Array.isArray(kids(n)) && kids(n).length,
                          )
                        "
          ...
          <div class="mt-3 flex justify-end">
            <button
              class="btn-primary"
              :disabled="
                loadingExport || (scope === 'selected' && selectedCount === 0)
              "
              @click="doExport"
            >
              <i class="mdi mdi-package-variant-closed" /> {{ t('Export') }}
            </button>
          </div>
        ...
        <div class="rounded-lg border border-gray-100 p-4">
          <h4 class="mb-2 text-sm font-semibold text-gray-800">
            {{ t('Import from CC 1.3') }}
          </h4>
          <p class="mb-3 text-sm text-gray-600">
            {{
              t(
                'Upload a Common Cartridge 1.3 (.imscc or .zip) to import resources.',
              )
            }}
          </p>
          <input
          ...
          <div class="mt-3 flex justify-end">
            <button
              ...
              @click="doImport"
            >
              <i class="mdi mdi-package-down" /> {{ t('Import') }}
            </button>
          </div>
        </div>
      </div>
...
function isFolder(node) {
  const ft = String(
    node?.extra?.filetype || node?.extra?.file_type || '',
  ).toLowerCase()
...
async function doExport() {
  ...
  try {
    ...
    if (scope.value === 'selected') {
      ...
      const total = Object.values(resSel).reduce(
      ...
      if (total === 0) {
        throw new Error(
          t(
            'Please select at least one exportable resource (documents, web links, or discussions).',
          ),
        )
      }
    ...
    error.value =
      e?.response?.data?.error || e.message || t('Failed to export.')
  } finally {
...
async function doImport() {
  ...
  try {
    ...
    error.value =
      e?.response?.data?.error || e.message || t('Failed to import.')
  } finally {
