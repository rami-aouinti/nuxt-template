<template>
  <div class="space-y-6">
    ...
    <section
      v-if="step === 1"
      class="rounded-lg border border-gray-25 p-4 space-y-4"
    >
      ...
      <label class="flex items-center gap-2">
        <input v-model="recycleOption" type="radio" value="full_recycle" />
        <span class="text-sm text-gray-90">{{
          t('Delete everything (irreversible)')
        }}</span>
      </label>
      <label class="flex items-center gap-2">
        <input v-model="recycleOption" type="radio" value="select_items" />
        <span class="text-sm text-gray-90">{{
      ...
      <div v-if="recycleOption === 'full_recycle'" class="mt-2">
        <label class="block text-xs font-medium text-gray-60 mb-1">{{
          t('Type the course code to confirm')
        }}</label>
        <input
      ...
      <CMAlert
        ...
        :text="
          t(
            'This will remove or reset selected resources. Action cannot be undone.',
          )
        "
      />

      <div class="flex justify-end gap-3">
        <button class="btn-primary" :disabled="loading" @click="nextFromStep1">
          <i
            class="mdi"
            :class="
              recycleOption === 'select_items'
                ? 'mdi-arrow-right'
                : 'mdi-recycle'
            "
          />
    ...
    <section v-if="step === 2" class="space-y-4">
      ...
      <div class="flex justify-between">
        ...
        <button class="btn-danger" :disabled="loading" @click="doRecycle">
          <i class="mdi mdi-recycle" /> {{ t('Recycle selected') }}
        </button>
      </div>
    </section>
...
async function doRecycle() {
  ...
  try {
    ...
    const payload = {
      ...
      confirm:
        recycleOption.value === 'full_recycle' ? confirm.value : undefined,
      resources:
        recycleOption.value === 'select_items' ? selections.value : undefined,
    }
