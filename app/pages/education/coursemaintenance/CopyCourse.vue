<template>
  <div class="space-y-6 cm-copy-course">
    ...
    <div class="flex items-center gap-3">
      <Step :index="1" :current="step" :label="t('Select source course')" />
      <Line />
      <Step :index="2" :current="step" :label="t('Options')" />
      <Line />
    ...
    <section v-if="step === 1" class="grid grid-cols-1 gap-4 lg:grid-cols-2">
      <div class="rounded-lg border border-gray-25 p-4">
        <h3 class="mb-3 text-sm font-semibold text-gray-90">
          {{ t('Source') }}
        </h3>

        <div v-if="useUnifiedPicker" class="space-y-2">
          <BaseSearchSelect
            v-model="sourceCourseId"
            :options="courseOptions"
            :label="t('Course to copy from')"
            :placeholder="t('Search by code or title…')"
            :empty-message="t('No courses match your search.')"
            :clearable="true"
            :virtual="true"
        ...
        <div v-else class="space-y-2">
          ...
          <p v-if="!filteredCourses.length" class="text-xs text-gray-50">
      ...
      <div class="rounded-lg border border-gray-25 p-4">
        ...
        <div class="space-y-3">
          <label class="flex items-center gap-2">
            <input v-model="copyOption" type="radio" value="full_copy" />
            <span class="text-sm text-gray-90">{{
              t('Everything (full copy)')
            }}</span>
          </label>
          <label class="flex items-center gap-2">
            <input v-model="copyOption" type="radio" value="select_items" />
            <span class="text-sm text-gray-90">{{
              t('Let me select learning objects')
            }}</span>
          </label>

          <div class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
            <label class="flex items-center gap-2">
              <input v-model="includeUsers" type="checkbox" />
              <span class="text-sm text-gray-90">{{
                t('Include users (enrollments)')
              }}</span>
            </label>
            <label class="flex items-center gap-2">
              <input v-model="resetDates" type="checkbox" />
      ...
      <div class="col-span-full flex justify-end gap-3">
        <button class="btn-secondary" :disabled="loading" @click="resetAll">
          <i class="mdi mdi-refresh" /> {{ t('Reset') }}
        </button>
        <button
          class="btn-primary"
          ...
          @click="nextFromStep1"
        >
          <i class="mdi mdi-arrow-right" /> {{ t('Continue') }}
        </button>
      </div>
    </section>
    ...
    <section v-if="step === 2" class="space-y-4">
      <CMInfo :title="t('Ready to copy')">
        <template #body>
          <p class="text-sm text-gray-50">
            {{
              t(
                'We will copy content from the selected course into this course. You can go back to change options.',
              )
            }}
          </p>
        </template>
      ...
      <div class="flex justify-between">
        <button class="btn-secondary" :disabled="loading" @click="step = 1">
          <i class="mdi mdi-arrow-left" /> {{ t('Back') }}
        </button>
        <button class="btn-primary" :disabled="loading" @click="doCopy">
          <i class="mdi mdi-content-copy" /> {{ t('Start copy') }}
        </button>
      </div>
    </section>
    ...
    <section v-if="step === 3" class="space-y-4">
      <CMAlert v-for="n in notices" :key="n" type="warning" :text="n" />

      <ResourceSelector
        v-model="selections"
        :groups="tree"
        :title="t('Select resources to copy')"
        :empty-text="t('No resources available in this course.')"
      />
      ...
      <div class="flex justify-between">
        ...
        <button class="btn-primary" :disabled="loading" @click="doCopy">
          <i class="mdi mdi-content-copy" /> {{ t('Copy selected') }}
        </button>
      </div>
    </section>
...
import ResourceSelector from '../../../components/education/coursemaintenance/ResourceSelector.vue'
import BaseSearchSelect from '../../../components/education/basecomponents/BaseSearchSelect.vue'
...
async function bootstrap() {
  ...
  try {
    ...
    sameFileNameOption.value =
      typeof data?.defaults?.sameFileNameOption !== 'undefined'
        ? data.defaults.sameFileNameOption
        : 2
  } catch (e) {
...
async function nextFromStep1() {
  error.value = ''
  try {
    if (!sourceCourseId.value)
      throw new Error(t('Please select a source course.'))
    loading.value = true

    if (copyOption.value === 'select_items') {
      // Get resource tree of source; ResourceSelector normalizes internally
      const data = await svc.fetchCopyResources(
        node.value,
        sourceCourseId.value,
      )
    ...
    error.value =
      e?.response?.data?.error || e?.message || t('Error loading resources.')
  } finally {
    loading.value = false
  }
}
  ...
  try {
    ...
    if (res.redirectUrl) {
      ...
      for (const k of ['gradebook', 'origin'])
        if (route.query[k] != null) keep[k] = route.query[k]
...
export default {
  components: {
    ...
    Line: {
      name: 'Line',
      template: `<div class="h-px flex-1 bg-gray-25"></div>`,
    },
    CMLoader: {
      name: 'CMLoader',
      template: `
