...
import GroupDiscussions from '../../../components/education/usergroup/GroupDiscussions.vue'
import GroupMembers from '../../../components/education/usergroup/GroupMembers.vue'
...
const joinGroup = async (delta) => {
  try {
    const response = await socialService.joinGroup(
      user.value.id,
      groupInfo.value.id,
    )
    if (response.success) {
