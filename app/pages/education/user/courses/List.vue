...
watch(result, (newResult) => {
  if (newResult?.courseRelUsers) {
    const newCourses = newResult.courseRelUsers.edges.map(
      ({ node }) => node.course,
    )
  ...
  setTimeout(() => {
    fetchMore({
      ...
      updateQuery: (previousResult, { fetchMoreResult }) => {
        ...
        const newCourses = fetchMoreResult.courseRelUsers.edges.map(
          ({ node }) => node.course,
        )
        const filteredCourses = newCourses.filter(
          (newCourse) =>
        ...
        return {
          ...
          courseRelUsers: {
            ...
            edges: [
              ...previousResult.courseRelUsers.edges,
              ...fetchMoreResult.courseRelUsers.edges,
            ],
          },
        }
      },
