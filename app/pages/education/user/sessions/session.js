...
export function useSession(type) {
  ...
  function loadUncategorizedSessions(sessions) {
    uncategorizedSessions.value.push(
      ...sessions.filter((session) => isEmpty(session.category)),
    )
  }
      ...
      if (session.category) {
        const alreadyAdded =
          categories.value.findIndex(
            (cat) => cat['@id'] === session.category['@id'],
          ) >= 0
  ...
  function loadCategoriesWithSessions(sessions) {
    sessions.forEach(function (session) {
      ...
      if (categoriesWithSessions.value.has(session.category['@id'])) {
        sessionsInCategory = categoriesWithSessions.value.get(
          session.category['@id'],
        ).sessions
      }
      ...
      categoriesWithSessions.value.set(session.category['@id'], {
  ...
  async function getSessions() {
    if (securityStore.isAuthenticated) {
      ...
      try {
        const { items, nextPageParams } =
          await sessionService.findUserSubscriptions(
