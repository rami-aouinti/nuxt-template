      ...
      <div>
        <div v-if="legalStatus.isAccepted">
          <p>
            ...
            <BaseIcon class="text-green-500" icon="check" size="small" />
          </p>
          ...
          <div v-if="termsAndConditions">
            ...
            >
              <BaseTextArea
                ...
                :label="
                  t('Explain in this box why you want your account deleted')
                "
                required
              />
              <LayoutFormButtons>
  ...
  >
    <div class="overflow-y-auto max-h-[70vh] pr-2">
      ...
      >
        ...
        <div class="mb-2" v-html="term.content" />
...
import BaseTextArea from '../../../components/education/basecomponents/BaseTextArea.vue'
import LayoutFormButtons from '../../../components/education/layout/LayoutFormButtons.vue'
...
async function fetchPersonalData() {
  ...
  try {
    personalData.data = await socialService.fetchPersonalData(
...
async function fetchTermsAndConditions() {
  ...
  try {
    termsAndConditions.value = await socialService.fetchTermsAndConditions(
      securityStore.user.id,
    )
  } catch (error) {
...
async function fetchLegalStatus() {
  ...
  try {
    const legalStatusData = await socialService.fetchLegalStatus(
      securityStore.user.id,
    )
