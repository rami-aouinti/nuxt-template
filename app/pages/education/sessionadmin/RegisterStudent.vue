<template>
  <div class="p-6 space-y-6">
    ...
    <AppCard class="p-4 space-y-4" elevation="2" hover>
      ...
      <div v-if="matches.length" class="mt-6 border-t pt-4">
        <h4 class="font-medium text-gray-90">
          {{ t('Matching students') }} ({{ matches.length }})
        </h4>
          ...
          <tbody class="divide-y divide-gray-25">
            ...
            >
              ...
              <td class="px-4 py-2">
                <span
                  :class="
                    user.hasLocalAccess ? 'text-green-600' : 'text-yellow-600'
                  "
                >
                  {{
    ...
    >
      <div class="space-y-4">
        ...
        <div class="flex items-center gap-2">
          <Checkbox v-model="createForm.sendEmail" :binary="true" />
          <span>{{ t('Send access details to user by email') }}</span>
        </div>
        <div class="flex justify-end gap-2">
          <Button text :label="t('Cancel')" @click="showCreateModal = false" />
          <Button
            icon="pi pi-check"
            :label="t('Save')"
...
const platformSessionAdminAccessAllUrls = computed(
  () =>
    platformConfigStore.getSetting(
      'platform.session_admin_access_to_all_users_on_all_urls',
    ) === 'true',
)
...
async function handleCreateUser() {
  ...
  const loginIsEmail =
    platformConfigStore.getSetting('profile.login_is_email') === 'true'
  ...
  try {
    const newUser = await userService.createOnAccessUrl(
      window.access_url_id || 1,
      payload,
    )
    if (extraFieldKey && createForm.value[extraFieldKey]) {
      newUser.extra = {
