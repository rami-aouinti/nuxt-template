<template>
  <div class="p-6">
    ...
    <div v-if="loading && certificates.length === 0" class="text-gray-500">
      {{ t('Loading') }}...
    </div>
    ...
    <div v-else>
      <table class="w-full text-left border-collapse">
        ...
        <tbody>
          <tr v-for="cert in certificates" :key="cert.id" class="border-b">
...
async function loadMore() {
  loading.value = true
  const response = await courseService.getCompletedCourses(
    offset.value,
    limit.value,
  )
