<template>
  <div v-if="cat" class="max-w-lg">
    <BaseToolbar
      ><template #start
        ><h3 class="font-semibold">Edit folder</h3></template
      ></BaseToolbar
    >
    <BaseInputText
      id="catTitle2"
...
const backTo = computed(() => ({
  name:
    cat.value?.area === 'received' ? 'DropboxListReceived' : 'DropboxListSent',
  params: route.params,
}))

onMounted(async () => {
  const all = (await service.listCategories({ area: 'sent' })).concat(
    await service.listCategories({ area: 'received' }),
  )
