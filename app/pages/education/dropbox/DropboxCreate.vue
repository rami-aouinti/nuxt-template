    ...
    <div class="grid mt-5 gap-6" style="grid-template-columns: 1.6fr 1fr">
      ...
      <div>
        <div class="rounded-lg border border-gray-50 bg-white shadow-sm">
          <div class="px-4 py-3 border-b text-sm font-medium">
            {{ t('Files') }}
          </div>
          <div class="p-4">
            <div class="uppy-wrap">
        ...
        <div class="rounded-lg border border-gray-50 bg-white shadow-sm mt-6">
          ...
          <div class="p-4">
            ...
            <label class="inline-flex items-center gap-3 mt-3">
              <input id="overwrite" v-model="overwrite" type="checkbox" />
              <span class="text-sm">{{
                t('Overwrite previous versions with same name?')
              }}</span>
            </label>
          </div>
        </div>
        ...
        <div class="rounded-lg border border-gray-50 bg-white shadow-sm">
          ...
          <div class="p-4">
            ...
            <BaseSelect
              ...
              :class="{
                'ring-1 ring-red-500 rounded-md':
                  submitted && !hasSelectedRecipient,
              }"
            />
            <small class="text-gray-500 block mt-1">
              {{
                t(
                  'Tip: choose “— Just upload —” to store without sending to anyone.',
                )
              }}
            </small>
            <div
            ...
            >
              {{
                t(
                  'Please select at least one recipient (“— Just upload —” or a user).',
                )
              }}
            </div>
...
onMounted(() => {
  ...
  const syncPicked = () => {
    pickedFiles.value = Object.values(uppy.value?.getFiles?.() ?? {}).map(
      (f) => f.data,
    )
  }
...
const hasSelectedRecipient = computed(
  () => normalizedRecipientTokens.value.length > 0,
)
const canSubmit = computed(
  () => pickedFiles.value.length > 0 && hasSelectedRecipient.value,
)
