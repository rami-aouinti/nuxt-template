<template>
  <div>
    ...
    >
      <div>
        <i :class="chamiloIconToClass['folder-generic']" class="mr-1" />
    ...
    >
      ...
      <Column selection-mode="multiple" header-style="width:3.2rem" />
      <Column
        :header="t('Type')"
      ...
      >
        <template #body="{ data }">
          ...
          />
          <i v-else :class="chamiloIconToClass['file-generic']" />
        </template>
      </Column>
        ...
        <template #body="{ data }">
          ...
          <template v-else>
            <a
              ...
              rel="noopener"
            >
              <i :class="chamiloIconToClass['download']" />
            </a>
            <span class="font-semibold">{{ data.title }}</span>
            <div v-if="data.description" class="text-sm text-gray-500">
              {{ data.description }}
            </div>
          </template>
        </template>
      ...
      <Column field="sizeHuman" :header="t('Size')" header-style="width:8rem">
        <template #body="{ data }">
          <span v-if="data.kind === 'file'">{{ data.sizeHuman }}</span>
        </template>
      </Column>

      <Column :header="t('Visible to')" header-style="width:18rem">
        <template #body="{ data }">
    ...
    <!-- Create folder -->
    <BaseDialog
      v-model:is-visible="showCatDialog"
      :title="t('New folder')"
    ...
    <!-- Rename folder -->
    <BaseDialog
      v-model:is-visible="showRenameDialog"
    ...
    <DropboxFeedbackDialog
      v-if="feedbackFile"
      v-model:is-visible="showFeedback"
...
import DropboxUpdateDialog from './dropbox-update-dialog.vue'
...
const currentCat = computed(() =>
  cats.value.find((c) => c.id === categoryId.value),
)
const selectedFiles = computed(() =>
  selected.value.filter((r) => r.kind === 'file'),
)
...
async function load() {
  ...
  items.value = await service.listFiles({
    area: 'sent',
...
async function saveRename() {
  ...
  await service.renameCategory({
    id: renameTarget.value.catId,
    title: renameName.value.trim(),
    area: 'sent',
  })
  closeRenameDialog()
  await load()
...
async function deleteFolder(row) {
  ...
  if (
    !confirm(
      t('Delete this folder? All files inside will be permanently deleted.'),
    )
  )
    return
