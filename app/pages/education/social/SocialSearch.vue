  ...
  <div class="social-search p-2">
    ...
    <BaseCard class="mb-2">
      ...
      <div class="flex flex-col items-end">
        <div class="w-full flex justify-between items-center mb-2">
          <label class="mr-2" for="search-query">{{
            t('Users, Groups')
          }}</label>
          <BaseInputText
            id="search-query"
        ...
        >
          <div class="flex items-center">
            ...
            <span
              ...
              title="Offline"
            />
            <span :class="getRoleIcon(user.role)" class="mx-2" />
          </div>
          <div>
            <BaseButton
    ...
    >
      <div class="invitation-modal">
        ...
        />
        <button class="invitation-modal-send" @click="sendInvitation">
          Send message
        </button>
      </div>
    </div>
    ...
    >
      <div class="message-modal">
        <div class="message-modal-header">
          ...
          <button class="message-modal-close" @click="closeMessageModal">
            ✕
          </button>
        </div>
        <div class="message-modal-body">
          ...
          />
          <button class="message-modal-send" @click="sendMessage">
            {{ t('Send message') }}
          </button>
        </div>
      </div>
...
const headerTitle = computed(() => {
  return searchPerformed.value
    ? `${t('Results and feedback')} "${query.value}"`
    : t('Search')
})
const performSearch = async () => {
  try {
    ...
    const response = await fetch(
      `/social-network/search?query=${query.value}&type=${searchType.value}`,
    )
    ...
    if (searchType.value === 'user') {
      users.value = data.results.map((item) => ({
        ...
        showInvitationButton:
          (![3, 4, 10].includes(item.relationType) ||
            item.id !== user.value.id) &&
          !item.existingInvitations,
      }))
      groups.value = []
