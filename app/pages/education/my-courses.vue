...
async function loadCourses() {
  ...
  try {
    ...
    courses.value = items.map((course) => ({
      ...
      visibility:
        course.visibility || (course.isPublic ? t('Public') : t('Privé')),
    ...
    <div class="page-header">
      <div>
        ...
        <p class="text-body-2 text-medium-emphasis">
          {{
            t(
              'Liste, création et gestion des cours sur une seule page via des modals.',
            )
          }}
        </p>
      </div>
        ...
        <div class="d-flex align-center justify-space-between mb-3">
          <div>
            ...
            <div class="text-body-2 text-medium-emphasis">
              {{
                t(
                  'Pilotez les cours, les participants et les ressources depuis une table unique.',
                )
              }}
            </div>
          </div>
    ...
    >
      <div v-if="activeCourse" class="py-4 px-2">
        ...
        <div class="text-body-2 mb-3">
          {{ t('Code') }}: {{ activeCourse.code }}
        </div>
        <div class="text-body-2 mb-3">
          {{ t('Visibilité') }}: {{ activeCourse.visibility }}
        </div>
        <div class="text-body-2 mb-3">
          {{ t('Date de début') }}:
        ...
        <div class="text-body-2 text-medium-emphasis">
          {{
            t(
              'Utilisez les actions pour modifier, inviter ou documenter ce cours.',
            )
          }}
        </div>
      </div>
    ...
    >
      <div v-if="activeCourse" class="pa-2">
        ...
        <div class="d-flex justify-end mt-4 gap-2">
          ...
          <v-btn color="primary" @click="saveEditedCourse">{{
            t('Mettre à jour')
          }}</v-btn>
        </div>
      </div>
    ...
    >
      <div v-if="activeCourse" class="pa-2">
        ...
        <div class="d-flex justify-end mt-4 gap-2">
          ...
          <v-btn color="primary" @click="addUserToCourse">{{
            t('Ajouter')
          }}</v-btn>
        </div>
      </div>
    ...
    >
      <div v-if="activeCourse" class="pa-2">
        ...
        <div class="d-flex justify-end mt-4 gap-2">
          ...
          <v-btn color="primary" @click="addDocumentToCourse">{{
            t('Ajouter')
          }}</v-btn>
        </div>
      </div>
