<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <BaseIcon icon="back" size="big" :title="t('Back')" @click="goBack" />
    </div>
    <hr />
    ...
    <div v-if="allowText && !allowFile" class="space-y-4">
      <BaseInputText
      ...
      />
      <BaseButton :label="t('Submit')" type="primary" @click="submitText" />
    </div>

    <div v-else-if="allowFile && !allowText">
      <Dashboard :uppy="uppy" :props="{ width: '100%', height: 300 }" />
    </div>
    ...
    <div v-else class="space-y-4">
      ...
      <div v-else class="space-y-2">
        <Dashboard :uppy="uppy" :props="{ width: '100%', height: 200 }" />
      </div>

      <BaseButton :label="t('Submit')" type="primary" @click="submitMixed" />
    </div>
  </div>
</template>
...
async function loadPublicationTitle() {
  try {
    const { data } = await axios.get(
      `${ENTRYPOINT}c_student_publications/${publicationId}`,
      {
...
async function submitText() {
  ...
  try {
    await axios.post(
      `/api/c_student_publications/upload?${queryParams}`,
      formData,
      {
