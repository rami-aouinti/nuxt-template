<template>
  ...
  <div>
    <div v-if="!assignment" class="p-4 text-center text-gray-600">
      {{ t('Loading...') }}
    </div>
    ...
    <div v-else class="space-y-6">
      ...
      >
        ...
        <ul class="space-y-2">
          <li v-for="doc in addedDocuments" :key="doc.document.iid">
            <a
...
function fromApiLocal(str) {
  if (!str) return null
  const s = String(str).includes('T')
    ? String(str)
    : String(str).replace(' ', 'T')
  return new Date(s)
}

const expiresOnDate = computed(() =>
  fromApiLocal(assignment.value?.assignment?.expiresOn),
)
const endsOnDate = computed(() =>
  fromApiLocal(assignment.value?.assignment?.endsOn),
)
const isAfterEndDate = computed(() =>
  endsOnDate.value ? new Date() > endsOnDate.value : false,
)
const isAfterDeadline = isAfterEndDate

const allowTextFlag = computed(
  () =>
    assignment.value?.allowTextAssignment === 0 ||
    assignment.value?.allowTextAssignment === 1,
)

const allowFileFlag = computed(
  () =>
    assignment.value?.allowTextAssignment === 0 ||
    assignment.value?.allowTextAssignment === 2,
)
...
onMounted(async () => {
  assignment.value = await cStudentPublicationService.getAssignmentMetadata(
    assignmentId,
    cid,
    sid,
    gid,
...
async function exportPdf() {
  if (!isTeacherUI.value) return
  try {
    const data = await cStudentPublicationService.exportAssignmentPdf(
      assignmentId,
      cid,
      sid,
      gid,
...
async function uploadCorrections() {
  ...
  input.addEventListener('change', async () => {
    ...
    try {
      const res = await cStudentPublicationService.uploadCorrectionsPackage(
        assignmentId,
        file,
      )
      notification.showSuccessNotification(
        t(
          `Corrections uploaded: ${res.uploaded ?? 0}. Skipped: ${res.skipped ?? 0}.`,
        ),
      )
...
async function deleteAllCorrections() {
  ...
  try {
    await cStudentPublicationService.deleteAllCorrections(
      assignmentId,
      cid,
      sid,
    )
