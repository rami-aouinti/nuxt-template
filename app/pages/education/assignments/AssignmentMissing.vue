<template>
  <div>
    ...
    <h1 class="text-xl font-bold">
      {{ t('Users without submission for assignment') }}:
      {{ assignment?.title || `#${id}` }}
    </h1>

    <div v-if="loading" class="flex justify-center my-10 m-4">
      <ProgressSpinner />
    </div>

    <BaseTable v-else-if="users.length" :values="users" data-key="id">
...
onMounted(async () => {
  ...
  try {
    ...
    assignment.value = await cstudentpublication.getAssignmentMetadata(
      props.id,
      cid,
      sid,
      gid,
...
async function sendEmailToAll() {
  try {
    ...
    notification.showSuccessNotification(
      t('Email sent to all unsubmitted users.'),
    )
  } catch (error) {
