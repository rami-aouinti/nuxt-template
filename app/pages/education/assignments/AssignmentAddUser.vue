<template>
  <div>
    ...
    <h1 class="text-2xl font-bold">
      {{ t('Add users') }} - {{ publicationTitle }}
    </h1>

    <div class="m-4">
      <h2 class="text-xl font-semibold mb-2">{{ t('Users added') }}</h2>
      <div v-if="isLoadingAdded" class="text-center text-gray-500 py-4">
...
async function loadAddedUsers() {
  ...
  try {
    const response = await axios.get(
      `${ENTRYPOINT}c_student_publication_rel_users`,
      {
        params: {
...
async function loadAvailableUsers() {
  ...
  try {
    ...
    if (sid > 0) {
      ...
      params.course = cid
      const response = await axios.get(
        `${ENTRYPOINT}session_rel_course_rel_users`,
        { params },
      )
      ...
      params.course = cid
      const response = await axios.get(`${ENTRYPOINT}course_rel_users`, {
        params,
      })
      const currentUserIds = new Set(addedUsers.value.map((u) => u.user['@id']))
