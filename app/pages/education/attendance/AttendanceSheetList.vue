<template>
  ...
  <div class="attendance-page p-4">
    ...
    <div v-if="isLoading" class="flex justify-center items-center h-64">
      <div class="loader" />
      <span class="ml-4 text-lg text-primary">{{
    ...
    <div v-else>
      <div v-if="isStudentUI">
        <h2 class="text-xl font-semibold mb-4">
          {{ t('Report of attendance sheets') }}
        </h2>

        <div v-if="filteredDates.length > 0" class="mb-4 text-sm text-gray-700">
          {{ t('To attend') }}
        ...
        >
          <div class="flex items-center">
            <template v-if="allowMultilevelGrading">
              <div
                :class="
                  getStateClass(attendanceData[`${currentUserId}-${date.id}`])
                "
                class="w-10 h-10 rounded-full mr-2"
                :title="
                  getStateLabel(
                    parseInt(attendanceData[`${currentUserId}-${date.id}`]),
                  )
                "
              />
            </template>
      ...
      <div v-else>
        ...
        <div
          class="p-4 mb-4 text-primary bg-gray-15 border border-gray-25 rounded"
        >
          <p>
            {{
              t(
        ...
        <div class="relative flex">
          <!-- Fixed user column -->
          <div class="overflow-hidden flex-shrink-0" style="width: 520px">
            <table class="w-full border-collapse">
              <thead>
                <tr class="bg-gray-15 h-28">
                  <th class="p-3 border border-gray-25 text-left">#</th>
                  <th class="p-3 border border-gray-25 text-left">
                    {{ t('Photo') }}
                  </th>
                  <th class="p-3 border border-gray-25 text-left">
                    {{ t('Last name') }}
                  </th>
                  <th class="p-3 border border-gray-25 text-left w-32">
                    {{ t('First name') }}
                  </th>
                  <th class="p-3 border border-gray-25 text-left">
                    {{ t('Not attended') }}
            ...
            <table class="w-full border-collapse">
              <thead>
                <tr class="bg-gray-15 h-28">
                  <template
                    v-if="
                      filteredDates.length === 0 && selectedFilter === 'today'
                    "
                  >
                    <th
                      colspan="100"
                    ...
                    >
                      <div class="flex justify-center items-center gap-4">
                        <BaseIcon icon="calendar" class="text-warning" />
                        <span>{{
                          t(
                            'There is no class scheduled today, try picking another day or add your attendance entry yourself using the action icons.',
                          )
                  ...
                  <template v-else>
                    <th
                      ...
                      :class="{
                        'bg-gray-200 cursor-not-allowed': isColumnLocked(
                          date.id,
                        ),
                      }"
                    >
                      <div class="flex flex-col items-center">
                        ...
                        <div v-if="isTeacherUI" class="flex gap-2 mt-1">
                          <BaseIcon
                            icon="view-table"
                            size="normal"
                ...
                <template v-else>
                  ...
                  >
                    <td
                      ...
                      :class="{
                        'bg-gray-200': isColumnLocked(date.id) || !canEdit,
                      }"
                    >
                      <div
                      ...
                      >
                        <template v-if="!allowMultilevelGrading">
                          <input
                            ...
                            :checked="
                              attendanceData[`${user.id}-${date.id}`] === 1
                            "
                            disabled
                          />
                        </template>
                        <template v-else>
                          <div
                            :class="
                              getStateClass(
                                attendanceData[`${user.id}-${date.id}`],
                              )
                            "
                      ...
                      >
                        ...
                        >
                          <div class="w-5 h-5 rounded-full bg-gray-30" />
        ...
        <div class="mt-4 flex justify-end">
          <BaseButton
            v-if="
              canEdit && filteredDates.some((date) => !isColumnLocked(date.id))
            "
          ...
          />
          <div v-if="isSaving" class="ml-2 loader" />
        </div>
      </div>

      <!-- Comment Dialog -->
      <BaseDialog v-model:is-visible="showCommentDialog" title="Add Comment">
        <textarea
      ...
      <!-- Signature Dialog -->
      <BaseDialog
        v-model:is-visible="showSignatureDialog"
        :title="t('Signature')"
      >
      ...
      <BaseDialog v-model:is-visible="showQrDialog" title="QR Code">
...
const isTeacherUI = computed(
  () => isTeacherUser.value && !platformConfigStore.isStudentViewActive,
)
const isStudentUI = computed(
  () => !isTeacherUser.value || platformConfigStore.isStudentViewActive,
)
...
const allowComments = computed(
  () =>
    platformConfigStore.getSetting('attendance.attendance_allow_comments') ===
    'true',
)
...
const signedCount = computed(
  () =>
    filteredDates.value.filter(
      (d) => attendanceData.value[`${currentUserId.value}-${d.id}`] === 1,
    ).length,
)
...
const isTodayScheduled = computed(() =>
  attendanceDates.value.some((date) => date.label.includes(todayDate)),
)
...
const filteredAttendanceSheets = computed(() => {
  if (isStudentUI.value) {
    ...
    return attendanceSheetUsers.value.filter(
  ...
  filteredAttendanceSheets.value.forEach((user) => {
    filteredDates.value.forEach((date) => {
      ...
      preparedData.push({
        ...
        calendarId: date.id,
        presence:
          attendanceData.value[key] !== undefined
            ? attendanceData.value[key]
            : null,
...
const fetchAttendanceSheetUsers = async (attendanceId) => {
  ...
  try {
    ...
    const users = await attendanceService.getAttendanceSheetUsers(
      attendanceId,
      params,
    )
...
const filterAttendanceSheets = () => {
  if (selectedFilter.value === 'all') {
    ...
    filteredDates.value = attendanceDates.value.filter(
      (date) => date.id === parseInt(selectedFilter.value, 10),
    )
  }
}
...
onMounted(async () => {
  ...
  if (isTeacherUI.value) {
    ...
    filteredDates.value = attendanceDates.value.filter(
      (d) =>
        attendanceData.value[`${userId}-${d.id}`] !== undefined &&
        attendanceData.value[`${userId}-${d.id}`] !== null,
    )
  }
})
...
const ATTENDANCE_STATES_BY_ID = Object.values(ATTENDANCE_STATES).reduce(
  (acc, state) => {
    acc[state.id] = state
    return acc
  },
...
const toggleAttendanceState = (userId, dateId) => {
  if (!canEdit.value) return
  if (!allowMultilevelGrading.value) {
    attendanceData.value[`${userId}-${dateId}`] =
      attendanceData.value[`${userId}-${dateId}`] === 1 ? 0 : 1
  } else {
    const currentState = attendanceData.value[`${userId}-${dateId}`] || 0
    attendanceData.value[`${userId}-${dateId}`] =
      (currentState + 1) % Object.keys(ATTENDANCE_STATES).length
  }
}
...
const exportToPdf = async () => {
  if (!isTeacherUI.value) return
  try {
    const blob = await attendanceService.exportAttendanceToPdf(
      route.params.id,
      {
        cid,
        sid,
...
const exportToXls = async () => {
  if (!isTeacherUI.value) return
  try {
    const blob = await attendanceService.exportAttendanceToXls(
      route.params.id,
      {
        cid,
        sid,
...
const generateQrCode = async () => {
  ...
  try {
    ...
    qrImageUrl.value = window.URL.createObjectURL(
