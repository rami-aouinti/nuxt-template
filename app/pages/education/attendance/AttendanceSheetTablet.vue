<template>
  ...
  <div class="p-4 space-y-4">
    ...
    <div v-else class="space-y-4">
      ...
      <div v-if="isLocked" class="p-3 rounded border bg-gray-15 text-gray-700">
        ...
        <table class="w-full border-collapse">
          <thead>
            <tr class="bg-gray-15">
              ...
              <th v-if="allowComments" class="p-3 text-center">
                {{ t('Comment') }}
              </th>
              <th v-if="enableSignature" class="p-3 text-center">
                {{ t('Signature') }}
              </th>
            </tr>
          </thead>
          <tbody>
            ...
            >
              ...
              <td v-if="allowComments" class="p-3 text-center">
                <BaseButton
                  icon="comment"
                  size="small"
                  type="info"
              ...
              <td v-if="enableSignature" class="p-3 text-center">
                <BaseButton
                  icon="drawing"
                  size="small"
                  type="secondary"
      ...
      <!-- Comment Dialog -->
      <BaseDialog
        v-model:is-visible="showCommentDialog"
        :title="t('Add Comment')"
      >
        <textarea
      ...
      >
        <div class="relative w-full h-48">
          <canvas ref="signaturePad" class="border rounded w-full h-full" />
          <button
            v-if="canEdit && !isLocked"
...
const canEdit = computed(
  () =>
    isTeacherUser.value && !isStudentView.value && route.query.readonly !== '1',
)

const enableSignature = computed(
  () =>
    platformConfig.getSetting('attendance.enable_sign_attendance_sheet') ===
    'true',
)
const allowComments = computed(
  () =>
    platformConfig.getSetting('attendance.attendance_allow_comments') ===
    'true',
)
...
const load = async () => {
  ...
  try {
    ...
    // One-day sheet
    const payload = await attendanceService.getDateSheet(
      attendanceId.value,
      calendarId.value,
      { cid, sid, gid },
    )
    ...
    let lockedFromBackend =
      lockFlag === true || lockFlag === 1 || lockFlag === '1'
