<template>
  <div v-if="course" id="course-home" class="course-home">
    ...
    <div v-else class="flex flex-col gap-4">
      <div class="section-header section-header--h2">
        ...
        <p
          v-if="
            isAllowedToEdit &&
            (exerciseAutoLaunch === 1 || exerciseAutoLaunch === 2)
          "
          class="text-sm text-gray-600"
        >
          {{
        ...
        <p
          v-if="
            isAllowedToEdit && (forumAutoLaunch === 1 || forumAutoLaunch === 2)
          "
          class="text-sm text-gray-600"
        >
          {{
      ...
      <div v-if="isAllowedToEdit" class="section-header section-header--h6">
        <h6 v-t="'Tools'" />

        <div class="ml-auto">
          <BaseToggleButton
            :disabled="
              isSorting || isCustomizing || !allowEditToolVisibilityInSession
            "
            :model-value="false"
      ...
      <div id="course-tools" class="course-home__tools">
        <CourseTool
...
import BaseMenu from '../../../components/education/basecomponents/BaseMenu.vue'
import BaseToggleButton from '../../../components/education/basecomponents/BaseToggleButton.vue'
...
import CourseIntroduction from '../../../components/education/course/CourseIntroduction.vue'
...
courseService.loadCTools(course.value.id, session.value?.id).then((cTools) => {
  ...
  noAdminToolsIndex
    .reverse()
    .forEach((element) => tools.value.splice(element, 1))
...
onMounted(async () => {
  ...
  await courseSettingsStore.loadCourseSettings(
    course.value.id,
    session.value?.id,
  )
  documentAutoLaunch.value =
    parseInt(
      courseSettingsStore.getSetting('enable_document_auto_launch'),
      10,
    ) || 0
  exerciseAutoLaunch.value =
    parseInt(
      courseSettingsStore.getSetting('enable_exercise_auto_launch'),
      10,
    ) || 0
  lpAutoLaunch.value =
    parseInt(courseSettingsStore.getSetting('enable_lp_auto_launch'), 10) || 0
  forumAutoLaunch.value =
    parseInt(courseSettingsStore.getSetting('enable_forum_auto_launch'), 10) ||
    0
})
...
const onStudentViewChanged = async () => {
  ...
  courseService
    .loadCTools(course.value.id, session.value?.id)
    .then((cTools) => {
      tools.value = cTools.map((element) => ({
        ...element,
...
const allowEditToolVisibilityInSession = computed(() => {
  ...
  return isInASession
    ? 'true' ===
        getSetting.value('session.allow_edit_tool_visibility_in_session')
    : true
})
</script>
