<template>
  <div>
    ...
    <div
      v-if="!isLoading && !linksWithoutCategory.length && !categories.length"
    >
    ...
    <div v-if="!isLoading" class="flex flex-col gap-4">
      ...
      >
        ...
        <ul>
          <li v-for="link in linksWithoutCategory" :key="link.id" class="mb-4">
            <LinkItem
              :is-link-valid="linkValidationResults[link.iid]"
              :link="link"
      ...
      >
        <template #header>
          ...
          <p v-if="category.info.description">
...
import EmptyState from '../../../components/education/EmptyState.vue'
...
import LinkCategoryCard from '../../../components/education/links/LinkCategoryCard.vue'
...
} from '../../../components/education/links/linkVisibility'
...
const isAllowedToEdit = ref(
  securityStore.isAdmin || securityStore.isCurrentTeacher,
)
...
async function reconcileEditGate() {
  try {
    ...
    isAllowedToEdit.value = Boolean(
      allowed || securityStore.isAdmin || securityStore.isCurrentTeacher,
    )
  } catch {
    isAllowedToEdit.value = Boolean(
      securityStore.isAdmin || securityStore.isCurrentTeacher,
...
async function toggleVisibility(link) {
  ...
  try {
    ...
    const updatedLink = await linkService.toggleLinkVisibility(
      link.iid,
      newVisible,
      cid,
      sid,
    ...
    notifications.showErrorNotification(
      t('Could not change visibility of link'),
    )
  } finally {
...
async function deleteCategory() {
  ...
  try {
    ...
    notifications.showErrorNotification(
      t('Could not change visibility of category'),
    )
  }
}
...
async function toggleCategoryVisibility(category) {
  ...
  try {
    ...
    category.info.visible = visibilityFromBoolean(
      updatedLinkCategory.linkCategoryVisible,
    )
