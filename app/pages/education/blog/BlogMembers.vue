<template>
  <div class="space-y-4">
    <BaseToolbar>
      <template #start
        ><h3 class="text-lg font-semibold m-0">{{ t('Members') }}</h3></template
      >
      <template #end>
    ...
    <div class="rounded-lg border bg-white shadow-sm">
      ...
      <div v-if="loadingMembers" class="p-4">
        <div class="animate-pulse h-4 w-40 bg-gray-100 rounded" />
      </div>
    ...
    <div v-if="showPicker" class="fixed inset-0 z-30">
      <div class="absolute inset-0 bg-black/30" @click="closePicker" />
        ...
        <div class="w-full max-w-2xl rounded-xl bg-white shadow-xl border p-4">
          ...
          <div class="flex items-center gap-2 mb-3">
            <BaseInputText
              id="pool-q"
              v-model="poolQ"
              :placeholder="t('Search users in course/session…')"
              label=""
            />
          ...
          <div class="border rounded">
            <div v-if="loadingPool" class="p-4">
              <div class="animate-pulse h-4 w-56 bg-gray-100 rounded" />
            </div>
            <div
