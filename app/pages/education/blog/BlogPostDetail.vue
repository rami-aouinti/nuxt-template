  ...
  <div class="grid gap-6 lg:grid-cols-[2fr,1fr]">
    <div class="rounded-lg border bg-white shadow-sm p-5">
      <template v-if="!loading">
        ...
        <div class="prose max-w-none" v-html="post.fullText" />
        ...
        <div class="mt-6 flex items-center gap-3 relative">
          <span class="text-sm text-gray-600" aria-live="polite">
            {{ t('Rating') }}: {{ rating.average.toFixed(1) }} ★ ({{
              rating.count
            }})
          </span>
          ...
          >
            <div class="text-xs text-gray-500 mb-2">
              {{ t('Select a score') }} (1–{{ ratingScaleMax }})
            </div>
            ...
            <div class="grid grid-cols-5 gap-2">
              <button
                ...
                :key="n"
                type="button"
                class="h-9 rounded-lg border text-sm transition hover:bg-yellow-50 hover:border-yellow-300 focus:outline-none focus:ring-2 focus:ring-yellow-300"
      ...
      <div v-else class="animate-pulse space-y-3">
        <div class="h-6 w-60 bg-gray-200 rounded" />
    ...
    <div class="rounded-lg border bg-white shadow-sm p-5">
      ...
      <div v-if="loadingComments" class="space-y-3">
        <div v-for="i in 3" :key="i" class="animate-pulse">
          <div class="h-4 w-40 bg-gray-100 rounded" />
          <div class="h-3 w-60 bg-gray-100 rounded mt-1" />
        </div>
      </div>
      <div v-else-if="!comments.length" class="text-sm text-gray-500">
        {{ t('No comments yet.') }}
      </div>
    ...
    <CommentDialog
      ...
      :initial-text="editCommentText"
      :dialog-title="t('Edit Comment')"
    ...
    <PostCreateDialog
      ...
      :initial-full-text="editText"
      :dialog-title="t('Edit Post')"
      :confirm-label="t('Save')"
      :header-icon="'pencil'"
      :show-files="false"
...
import CommentDialog from '../../../components/education/blog/CommentDialog.vue'
import PostCreateDialog from '../../../components/education/blog/PostCreateDialog.vue'
...
const isTeacherOrAdmin = computed(
  () => securityStore.isTeacher || securityStore.isAdmin,
)
