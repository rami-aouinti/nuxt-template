<template>
  <div v-if="isAdminOrTeacher" class="w-full blog-admin">
    ...
    <BaseToolbar class="sticky top-0 z-10 bg-[var(--surface-card,#fff)]">
      <template #start>
        <div class="flex items-center gap-2">
          <i class="mdi mdi-view-grid-plus text-xl text-primary" />
    ...
    <div class="controls">
      ...
      <label class="inline-flex items-center text-sm gap-2">
        <input v-model="onlyVisible" type="checkbox" />
        ...
        <div class="card-head">
          <div>
            ...
            <RouterLink
              ...
              :to="{
                name: 'BlogPosts',
                params: { ...$route.params, blogId: proj.id },
                query: $route.query,
              }"
            >
    ...
    <ProjectCreateDialog
      :key="createDialogKey"
      v-model:is-visible="showCreate"
      @submitted="createProject"
    />
    <BaseDialog
      v-model:is-visible="showRename"
  ...
  <div v-else class="p-8 text-center text-gray-600">
    <div class="inline-flex items-center gap-2 text-red-600 font-semibold mb-2">
      <i class="mdi mdi-lock-alert-outline text-2xl" />
