<template>
  ...
  <BaseTable :values="templates" data-key="id">
    <Column header-style="width: 80px">
      <template #body="{ data }">
        <BaseIcon icon="template" size="normal" class="text-primary" />
      </template>
    </Column>
  ...
  >
    <pre
      class="text-xs whitespace-pre-wrap break-words max-h-[400px] overflow-auto"
      >{{ selectedJson }}
    </pre>
  </BaseDialogConfirmCancel>
...
function getBlocksCount(template) {
  try {
    const parsed = JSON.parse(template.layout)
    return (
      parsed?.page?.layout?.columns?.reduce(
