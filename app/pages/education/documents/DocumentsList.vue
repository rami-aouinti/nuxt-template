<template>
  <SectionHeader v-if="securityStore.isAuthenticated" :title="t('Documents')">
    <BaseButton
      ...
      <template #body="slotProps">
        <div class="flex flex-row justify-end gap-2">
          ...
          <BaseButton
            :disabled="
              !(
                slotProps.data.filetype === 'file' ||
                slotProps.data.filetype === 'video'
              )
            "
          ...
          <BaseButton
            ...
            :icon="
              ...
              slotProps.data.resourceLinkListFromEntity[0].visibility
                ? 'eye-on'
                : RESOURCE_LINK_DRAFT ===
                    slotProps.data.resourceLinkListFromEntity[0].visibility
                  ? 'eye-off'
          ...
          <BaseButton
            v-if="
              canEdit(slotProps.data) &&
              allowAccessUrlFiles &&
              isFile(slotProps.data) &&
  ...
  >
    ...
    <BaseButton
      ...
      :label="
        isDownloading ? t('In progress') : t('Download selected items as ZIP')
      "
      icon="download"
      type="primary"
    ...
    <form @submit.prevent="submitTemplateForm">
      <div class="p-float-label">
        <InputText
          ...
          required
        />
        <label v-t="'Name'" for="templateTitle" />
      </div>
      <small
  ...
  >
    <div class="confirmation-content">
      ...
      <ul class="pl-4 mb-4">
        <li v-for="lp in lpListWarning" :key="lp.lpId + lp.documentTitle">
...
import DocumentEntry from '../../../components/education/documents/DocumentEntry.vue'
...
import { useFileUtils } from '~/composables/education/fileUtils'
import BaseDialog from '../../../components/education/basecomponents/BaseDialog.vue'
import BaseChart from '../../../components/education/basecomponents/BaseChart.vue'
import DocumentAudioRecorder from '../../../components/education/documents/DocumentAudioRecorder.vue'
import { useNotification } from '~/composables/education/notification'
import { useSecurityStore } from '~/stores/securityStore'
import prettyBytes from 'pretty-bytes'
import BaseFileUpload from '../../../components/education/basecomponents/BaseFileUpload.vue'
...
const allowAccessUrlFiles = computed(
  () =>
    'false' !==
    platformConfigStore.getSetting('document.access_url_specific_files'),
)

const { t } = useI18n()
const { filters, options, onUpdateOptions, deleteItem } =
  useDatatableList('Documents')
...
const isCurrentTeacher = computed(
  () =>
    securityStore.isCurrentTeacher && !platformConfigStore.isStudentViewActive,
)
...
const canEdit = (item) => {
  ...
  const isSessionDocument =
    ...
    resourceLink.session['@id'] === `/api/sessions/${sid}`
...
watch(totalItems, (n) => {
  const def = Number(
    platformConfigStore.getSetting('display.table_default_row', 10),
  )
  if (def === 0 && n) {
...
async function confirmDeleteItem(itemToDelete) {
  try {
    const response = await axios.get(
      `/api/documents/${itemToDelete.iid}/lp-usage`,
    )
...
async function forceDeleteItem() {
  try {
    const docIdsToDelete = [
      ...new Set(lpListWarning.value.map((lp) => lp.documentId)),
    ]

    await Promise.all(
      docIdsToDelete.map((iid) => axios.delete(`/api/documents/${iid}`)),
    )
...
async function replaceDocument() {
  ...
  if (
    !(
      documentToReplace.value.filetype === 'file' ||
      documentToReplace.value.filetype === 'video'
    )
  ) {
  ...
  try {
    await axios.post(
      `/api/documents/${documentToReplace.value.iid}/replace`,
      formData,
      {
        headers: {
...
async function fetchFolders(nodeId = null, parentPath = '') {
  ...
  try {
    const nodesToFetch = [
      { id: nodeId || route.params.node || route.query.node, path: parentPath },
    ]
    let depth = 0
    const maxDepth = 5
    ...
    while (nodesToFetch.length > 0 && depth < maxDepth) {
      ...
      response.data['hydra:member'].forEach((folder) => {
        ...
        foldersList.push({
          label: fullPath,
          value:
            folder.resourceNode?.id || folder.resourceNodeId || folder['@id'],
        })
...
async function selectAsDefaultCertificate(certificate) {
  try {
    const response = await axios.patch(
      `/gradebook/set_default_certificate/${cid}/${certificate.iid}`,
    )
    if (response.status === 200) {
      loadDefaultCertificate()
      onUpdateOptions(options.value)
      notification.showSuccessNotification(
        t('Certificate set as default successfully'),
      )
    }
  } catch {
    notification.showErrorNotification(
      t('Error setting certificate as default'),
    )
  }
}
...
const isDocumentTemplate = async (documentId) => {
  try {
    const response = await axios.get(
      `/template/document-templates/${documentId}/is-template`,
    )
...
const getTemplateIcon = (documentId) => {
  ...
  return document && document.template
    ? 'template-selected'
...
const submitTemplateForm = async () => {
  ...
  if (!templateFormData.value.title || !selectedFile.value) {
    notification.showErrorNotification(
      t('The title and thumbnail are required.'),
    )
    return
  }
  ...
  try {
    ...
    const response = await axios.post(
      '/template/document-templates/create',
      formData,
      {
        headers: {
          'Content-Type': 'multipart/form-data',
