<template>
  <div class="p-4 space-y-8">
    ...
    <div class="space-y-6">
      ...
      <form class="flex flex-col space-y-4" @submit.prevent="uploadVariation">
...
async function fetchVariations() {
  ...
  try {
    ...
    const response = await axios.get(
      `/r/resource_files/${resourceNodeId}/variants`,
    )
...
async function uploadVariant(file, resourceNodeId, accessUrlId) {
  if (!resourceNodeId) {
    console.error(
      'ResourceNodeId is undefined. Check originalFile:',
      originalFile.value,
    )
    return
  }
  ...
  try {
    const response = await axios.post(
      '/api/resource_files/add_variant',
      formData,
    )
