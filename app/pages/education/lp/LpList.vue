<template>
  <div class="space-y-6">
      ...
      <div class="relative flex items-center gap-2">
        ...
        >
          <template #button>
            ...
            >
              <i class="mdi mdi-dots-vertical text-lg" aria-hidden />
            </button>
          </template>
          <template #menu>
            <div
              class="absolute right-0 z-50 w-56 bg-white border border-gray-25 rounded-xl shadow-md p-1"
            >
              <button
    ...
    >
      <div
        class="w-24 h-24 rounded-full bg-support-1 flex items-center justify-center mb-4 text-support-3"
      >
      ...
      <h3 class="text-base font-semibold">
        {{ t("You don't have learning paths.") }}
      </h3>
      <p class="text-body-2 text-gray-50 max-w-sm">
        {{
          t(
            'Create your first learning path to start organizing course content.',
          )
        }}
      </p>
      <button
        ...
        >
          <template #item="{ element }">
            <LpRowItem
              ...
              :can-export-scorm="canExportScorm"
              :can-export-pdf="canExportPdf"
              :can-auto-launch="canAutoLaunch"
...
import LpRowItem from '../../../components/education/lp/LpRowItem.vue'
import LpCategorySection from '../../../components/education/lp/LpCategorySection.vue'
import StudentViewButton from '../../../components/education/StudentViewButton.vue'

import BaseDropdownMenu from '../../../components/education/basecomponents/BaseDropdownMenu.vue'
...
const load = async () => {
  ...
  try {
    ...
    if (
      !allowed &&
      Array.isArray(roles) &&
      (roles.includes('ROLE_ADMIN') || roles.includes('ROLE_GLOBAL_ADMIN'))
    ) {
      allowed = true
    }
    ...
    items.value = raw.map((lp) => ({
      ...lp,
      coverUrl:
        hasImageRF(lp) && lp.contentUrl ? withCidSid(lp.contentUrl) : null,
    }))
...
function rebuildListsFromItems() {
  const source = canEdit.value
    ? items.value
    : items.value.filter(isVisibleForStudent)
...
const hasAnyVisible = computed(() => {
  ...
  const anyCat = Object.values(catLists.value).some(
...
async function onReorderCategory(cat, ids) {
  await nextTick()
  applyOrderWithinContext(
    (lp) => lp.category && lp.category.iid === cat.iid,
    ids,
  )
  try {
...
const ringValue = (val) =>
  Math.round(Math.min(100, Math.max(0, Number(val || 0))))
...
const handleTopMenu = (action, ev) => {
  ...
  const url =
    action === 'new'
      ? lpService.buildLegacyActionUrl('add_lp', {
          cid: courseId,
          sid: sidQ,
          node,
          gid,
      ...
      : action === 'category'
        ? lpService.buildLegacyActionUrl('add_lp_category', {
            cid: courseId,
            sid: sidQ,
            node,
            gid,
        ...
        : action === 'import'
          ...
          : action === 'rapid'
            ...
            : action === 'ai'
              ? lpService.buildLegacyActionUrl('ai_helper', {
                  cid: courseId,
                  sid: sidQ,
                  node,
                  gid,
...
const onSettings = (lp) =>
  (window.location.href = lpService.buildLegacyActionUrl(lp.iid, 'edit', {
    cid: cid.value,
    sid: sid.value,
  }))
const onBuild = (lp) =>
