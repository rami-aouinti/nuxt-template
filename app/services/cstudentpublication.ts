...
async function getAssignmentMetadata(assignmentId, cid, sid = 0, gid = 0) {
  ...
  const response = await axios.get(
    `${ENTRYPOINT}c_student_publications/${assignmentId}?${params}`,
  )
...
async function getAssignmentDetail({
  ...
  const params = {
    ...
    itemsPerPage,
    ...Object.fromEntries(
      Object.entries(order).map(([key, val]) => [`order[${key}]`, val]),
    ),
  }
...
async function getAssignmentDetailForTeacher({
  assignmentId,
  page = 1,
  itemsPerPage = 10,
  order = {},
  ...
  const response = await axios.get(
    `/assignments/${assignmentId}/submissions/teacher`,
    { params },
  )
  return response.data
}
...
async function uploadComment(
  ...
  const response = await axios.post(
    `${ENTRYPOINT}c_student_publication_comments/upload?${queryParams}`,
    formData,
    {
      headers: {
...
async function loadComments(submissionId) {
  try {
    const response = await axios.get(
      `${ENTRYPOINT}c_student_publication_comments?publication.iid=${submissionId}`,
    )
...
async function getUnsubmittedUsers(assignmentId) {
  ...
  const response = await axios.get(
    `/assignments/${assignmentId}/unsubmitted-users?${params}`,
  )
  return response.data['hydra:member']
}
...
async function sendEmailToUnsubmitted(assignmentId, queryParams = {}) {
  const params = new URLSearchParams(queryParams).toString()
  const response = await axios.post(
    `/assignments/${assignmentId}/unsubmitted-users/email?${params}`,
  )
  return response.data
}
...
async function downloadAssignments(assignmentId) {
  const response = await axios.get(
    `/assignments/${assignmentId}/download-package`,
    {
      responseType: 'blob',
    },
  )
