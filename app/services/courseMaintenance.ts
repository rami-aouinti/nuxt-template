...
function courseContextParams() {
  ...
  return {
    ...
    ...(fromStore.cid
      ? {}
      : pickNum('cid', 'cidReq')
        ? { cid: pickNum('cid', 'cidReq') }
        : {}),
    ...(fromStore.sid
      ? {}
      : pickNum('sid', 'id_session')
        ? { sid: pickNum('sid', 'id_session') }
        : {}),
    ...(fromStore.gid
      ? {}
      : pickNum('gid', 'gidReq')
        ? { gid: pickNum('gid', 'gidReq') }
        : {}),
  }
}
...
function resolveNodeFromPath() {
  ...
  let m = window.location.pathname.match(
    /\/resources\/course_maintenance\/(\d+)/,
  )
  if (m && m[1]) {
    const v = Number(m[1])
...
const base = {
  ...
  resources: (node, backupId) =>
    `/course_maintenance/${node}/import/${backupId}/resources`,
  restore: (node, backupId) =>
    `/course_maintenance/${node}/import/${backupId}/restore`,
  ...
  // Export Moodle (.mbz)
  moodleExportOptions: (node) =>
    `/course_maintenance/${node}/moodle/export/options`,
  moodleExportResources: (node) =>
    `/course_maintenance/${node}/moodle/export/resources`,
  moodleExportExecute: (node) =>
    `/course_maintenance/${node}/moodle/export/execute`,
  ...
  cc13ExportOptions: (node) =>
    `/course_maintenance/${node}/cc13/export/options`,
  cc13ExportResources: (node) =>
    `/course_maintenance/${node}/cc13/export/resources`,
  cc13ExportExecute: (node) =>
    `/course_maintenance/${node}/cc13/export/execute`,
...
async function getOptions(node = resolveNodeFromPath()) {
  const resp = await http.get(base.options(node), {
...
async function fetchResources(node = resolveNodeFromPath(), backupId) {
  const resp = await http.get(base.resources(node, backupId), {
    params: withCourseParams(),
  })
  return resp.data
}
async function restoreBackup(
  ...
  backupId,
  { importOption, sameFileNameOption, resources },
) {
  const payload = { importOption, sameFileNameOption }
  if (importOption === 'select_items') payload.resources = resources || {}
  const resp = await http.post(base.restore(node, backupId), payload, {
    params: withCourseParams(),
  })
  return resp.data
}
...
async function getRecycleOptions(node) {
  const r = await http.get(base.recycleOptions(node), {
    params: withCourseParams(),
  })
  return r.data
}
async function fetchRecycleResources(node) {
  const r = await http.get(base.recycleResources(node), {
    params: withCourseParams(),
  })
  return r.data
}
async function recycleExecute(node, payload) {
  const r = await http.post(base.recycleExecute(node), payload, {
    params: withCourseParams(),
  })
  return r.data
}
...
async function copyCourse(node = resolveNodeFromPath(), payload) {
  const resp = await http.post(`/course_maintenance/${node}/copy`, payload, {
    params: withCourseParams(),
  })
  return resp.data
}
...
async function recycleCourse(node = resolveNodeFromPath(), payload) {
  const resp = await http.post(base.recycleCourse(node), payload, {
    params: withCourseParams(),
  })
  return resp.data
}
async function deleteCourse(node = resolveNodeFromPath(), payloadOrConfirm) {
  const payload =
    typeof payloadOrConfirm === 'string'
      ? { confirm: payloadOrConfirm }
      : payloadOrConfirm || {}

  const resp = await http.post(base.deleteCourse(node), payload, {
    params: withCourseParams(),
  })
...
async function moodleExportOptions(node = resolveNodeFromPath()) {
  const resp = await http.get(base.moodleExportOptions(node), {
    params: withCourseParams(),
  })
  return resp.data
}
...
async function moodleExportResources(node = resolveNodeFromPath()) {
  const resp = await http.get(base.moodleExportResources(node), {
    params: withCourseParams(),
  })
  return resp.data
}
...
async function cc13ExportOptions(node = resolveNodeFromPath()) {
  const resp = await http.get(base.cc13ExportOptions(node), {
    params: withCourseParams(),
  })
  return resp.data
}
async function cc13ExportResources(node = resolveNodeFromPath()) {
  const resp = await http.get(base.cc13ExportResources(node), {
    params: withCourseParams(),
  })
  return resp.data
}
