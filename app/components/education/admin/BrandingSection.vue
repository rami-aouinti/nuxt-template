<template>
  <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
    <div
      class="md:col-span-2 p-3 rounded-lg border border-blue-100 bg-blue-50 text-blue-900 text-xs mb-2"
    >
    ...
    <!-- Header logo -->
    <div
      class="p-6 rounded-2xl shadow-sm border border-gray-100 bg-white space-y-6"
    >
      <div>
      ...
      <section class="rounded-xl border border-gray-100 bg-gray-20 p-4">
        <div
          class="text-[11px] font-medium uppercase tracking-wide opacity-70 mb-3"
        >
          {{ t('Effective preview') }}
        </div>
      ...
      <section class="space-y-3">
        ...
        <div class="flex items-center gap-3">
          <span
            class="inline-flex items-center rounded-full border border-gray-200 px-2 py-0.5 text-[11px] uppercase"
            >SVG</span
          >
          <img
            :key="'h-s-' + cacheBust"
          ...
          />
          <span v-if="headerSvgExists === false" class="text-xs opacity-60">{{
            t('No SVG uploaded')
          }}</span>
          <button
        ...
        <div class="flex items-center gap-3">
          ...
          />
          <span v-if="headerPngExists === false" class="text-xs opacity-60">{{
            t('No PNG uploaded')
          }}</span>
          <button
      ...
      <section class="grid grid-cols-1 gap-3">
        ...
        <div>
          <label class="text-xs block mb-1">{{
            t('Upload PNG for header (≤190×60)')
          }}</label>
          <input
            type="file"
            accept="image/png"
    ...
    >
      ...
      <section class="space-y-3">
        ...
        <div class="flex items-center gap-3">
          ...
          />
          <span v-if="emailSvgExists === false" class="text-xs opacity-60">{{
            t('No SVG uploaded')
          }}</span>
          <button
        ...
        <div class="flex items-center gap-3">
          ...
          />
          <span v-if="emailPngExists === false" class="text-xs opacity-60">{{
            t('No PNG uploaded')
          }}</span>
          <button
      ...
      <section class="grid grid-cols-1 gap-3">
        ...
        <div>
          <label class="text-xs block mb-1">{{
            t('Upload email PNG (~540 px width)')
          }}</label>
          <input
            type="file"
            accept="image/png"
...
import themeLogoService from '~/services/themeLogoService.js'
...
const headerSvgStrict = computed(() =>
  u(props.slug, 'images/header-logo.svg', { strict: true }),
)
const headerPngStrict = computed(() =>
  u(props.slug, 'images/header-logo.png', { strict: true }),
)
const emailSvgStrict = computed(() =>
  u(props.slug, 'images/email-logo.svg', { strict: true }),
)
const emailPngStrict = computed(() =>
  u(props.slug, 'images/email-logo.png', { strict: true }),
)

const headerSvgDefault = computed(() =>
  u(DEFAULT_THEME, 'images/header-logo.svg'),
)
const headerPngDefault = computed(() =>
  u(DEFAULT_THEME, 'images/header-logo.png'),
)
const emailSvgDefault = computed(() =>
  u(DEFAULT_THEME, 'images/email-logo.svg'),
)
const emailPngDefault = computed(() =>
  u(DEFAULT_THEME, 'images/email-logo.png'),
)
...
// existence flags
const hasHeaderCustom = computed(
  () => headerSvgExists.value === true || headerPngExists.value === true,
)
const hasEmailCustom = computed(
  () => emailSvgExists.value === true || emailPngExists.value === true,
)
  ...
  if (key === 'headerPng' || key === 'emailPng') {
    ...
    img.onload = () => {
      ...
      if (
        key === 'emailPng' &&
        opts.recommendW &&
        img.width !== opts.recommendW
      ) {
        console.warn(
          `Email PNG width is ${img.width}, recommended ${opts.recommendW}px`,
        )
      }
      files.value[key] = f
    }
