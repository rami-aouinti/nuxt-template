...
// Reactive colors
const colorPrimary = getColorTheme('--color-primary-base')
const colorPrimaryGradient = getColorTheme('--color-primary-gradient')
const colorPrimaryButtonText = getColorTheme('--color-primary-button-text')
const colorPrimaryButtonAlternativeText = getColorTheme(
  '--color-primary-button-alternative-text',
)

const colorSecondary = getColorTheme('--color-secondary-base')
const colorSecondaryGradient = getColorTheme('--color-secondary-gradient')
const colorSecondaryButtonText = getColorTheme('--color-secondary-button-text')

const colorTertiary = getColorTheme('--color-tertiary-base')
const colorTertiaryGradient = getColorTheme('--color-tertiary-gradient')
const colorTertiaryButtonText = getColorTheme('--color-tertiary-button-text')

const colorSuccess = getColorTheme('--color-success-base')
const colorSuccessGradient = getColorTheme('--color-success-gradient')
const colorSuccessButtonText = getColorTheme('--color-success-button-text')

const colorInfo = getColorTheme('--color-info-base')
const colorInfoGradient = getColorTheme('--color-info-gradient')
const colorInfoButtonText = getColorTheme('--color-info-button-text')

const colorWarning = getColorTheme('--color-warning-base')
const colorWarningGradient = getColorTheme('--color-warning-gradient')
const colorWarningButtonText = getColorTheme('--color-warning-button-text')

const colorDanger = getColorTheme('--color-danger-base')
const colorDangerGradient = getColorTheme('--color-danger-gradient')
const colorDangerButtonText = getColorTheme('--color-danger-button-text')

const formColor = getColorTheme('--color-form-base')
...
watch(
  ...
  (nv) =>
    (colorPrimaryButtonTextError.value = checkColorContrast(
      new Color('white'),
      nv,
    )),
)
...
watch(
  ...
  (nv) =>
    (colorPrimaryButtonAlternativeTextError.value = checkColorContrast(
      colorPrimary.value,
      nv,
    )),
)
...
watch(
  ...
  (nv) =>
    (colorSecondaryButtonTextError.value = checkColorContrast(
...
watch(
  ...
  (nv) =>
    (colorTertiaryButtonTextError.value = checkColorContrast(
      colorTertiary.value,
      nv,
    )),
)
...
watch(
  ...
  (nv) =>
    (colorSuccessButtonTextError.value = checkColorContrast(
      colorSuccess.value,
      nv,
    )),
)
...
watch(
  colorInfoButtonText,
  (nv) =>
    (colorInfoButtonTextError.value = checkColorContrast(colorInfo.value, nv)),
)
const colorWarningButtonTextError = ref('')
watch(
  colorWarningButtonText,
  (nv) =>
    (colorWarningButtonTextError.value = checkColorContrast(
      colorWarning.value,
      nv,
    )),
)
...
watch(
  ...
  (nv) =>
    (colorDangerButtonTextError.value = checkColorContrast(
      new Color('white'),
      nv,
    )),
)
...
watch(colorSecondary, (nv) => {
  ...
  colorSecondaryButtonTextError.value = checkColorContrast(
    nv,
    colorSecondaryButtonText.value,
  )
})
...
watch(colorSuccess, (nv) => {
  ...
  colorSuccessButtonTextError.value = checkColorContrast(
    nv,
    colorSuccessButtonText.value,
  )
})
watch(colorInfo, (nv) => {
  colorInfoGradient.value = makeGradient(nv)
  colorInfoButtonText.value = makeTextWithContrast(nv)
  colorInfoButtonTextError.value = checkColorContrast(
    nv,
    colorInfoButtonText.value,
  )
})
...
<template>
  ...
  <SectionHeader :title="t('Modify color theme')" size="6">
    <BaseButton
      :label="
        isAdvancedMode ? t('Hide advanced mode') : t('Show advanced mode')
      "
      icon="cog"
      type="black"
