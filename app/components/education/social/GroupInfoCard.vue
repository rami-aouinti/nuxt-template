<template>
  ...
  >
    <form @submit.prevent="submitGroupEdit">
      ...
      />
      <Button class="p-button-text" label="Close" @click="closeEditDialog" />
    </form>
  </Dialog>
</template>
...
const editGroupForm = ref({
  ...
  permissions: permissionsOptions.find(
    (option) => option.value === groupInfo.value.visibility,
  ),
    ...
    axios
      ...
      .then((response) => {
        if (selectedFile.value && response.data && response.data.id) {
          ...
          return axios.post(
            `/social-network/upload-group-picture/${response.data.id}`,
            formData,
            {
              headers: {
