    ...
    <ul v-if="groupInfo.isMember" class="menu-list">
      ...
      <li class="menu-item">
        <BaseAppLink :to="{ name: '', params: { group_id: groupInfo.id } }">
          <i aria-hidden="true" class="mdi mdi-account-multiple-outline" />
          {{ t('Waiting list') }}
        </BaseAppLink>
      </li>
      <li class="menu-item">
        <BaseAppLink
          :to="{ name: 'UserGroupInvite', params: { group_id: groupInfo.id } }"
        >
          <i aria-hidden="true" class="mdi mdi-account-plus" />
          {{ t('Invite friends') }}
        </BaseAppLink>
      </li>
      <li v-if="groupInfo.isAllowedToLeave" class="menu-item">
        <button @click="leaveGroup">
          <i aria-hidden="true" class="mdi mdi-exit-to-app" />
          {{ t('Leave group') }}
        </button>
      </li>
    </ul>
  ...
  try {
    ...
    if (response.data.success) {
      notification.showSuccessNotification(
        t('You have left the group successfully'),
      )
      router.push('/social')
    }
  } catch (error) {
