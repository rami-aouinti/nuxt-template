<template>
  <BaseCard plain>
    <div class="flex flex-col items-center p-4 user-profile-card">
      ...
      >
        <dl class="extra-info-list">
          <template v-for="item in extraInfo" :key="item.variable">
            <div v-if="item.value">
              <dt v-if="item.variable !== 'langue_cible'">
                {{ t(item.label) }}:
              </dt>
              <dd v-if="item.variable !== 'langue_cible'">
                {{ t(item.value) }}
              </dd>
      ...
      <Divider />
      <div v-if="pushEnabled" class="mt-4 w-full text-center">
        <p
        ...
        <div v-else>
          <template v-if="isSubscribed">
            <div class="flex flex-col items-center text-green-700">
              <i class="mdi mdi-bell-ring-outline text-4xl mb-2" />
          ...
          <template v-else>
            <div class="flex flex-col items-center text-red-700">
              <i class="mdi mdi-bell-off-outline text-4xl mb-2" />
              <p class="text-sm">
...
import { usePushSubscription } from '~/composables/usePushSubscription.js'
...
const showFullProfile = computed(
  () => isCurrentUser.value || securityStore.isAdmin,
)
