<template>
  <div class="mt-8">
    <h3 v-t="'Email Settings'" class="mb-4" />
    ...
    <BaseInputText
      ...
      :help-text="
        t(
          'The name that appears as the sender (next to the From e-mail address) when the platform sends an e-mail.',
        )
      "
...
async function testSmtp() {
  ...
  try {
    ...
    formData.append(
      'mailerFromEmail',
      installerData.value.stepData.mailerFromEmail,
    )
    formData.append(
      'mailerFromName',
      installerData.value.stepData.mailerFromName,
    )
    formData.append('mailerTestDestination', mailerTestDestination.value)

    const { data } = await axios.post(
      '/main/inc/ajax/install.ajax.php?a=test_mailer',
      formData,
    )
...
onMounted(() => {
  if (
    installerData.value.installType === 'new' &&
    installerData.value.stepData.smtpUniqueReplyTo === undefined
  ) {
