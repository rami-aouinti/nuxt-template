<template>
  <div class="w-full">
    <FloatLabel variant="on">
      <Dropdown
        ...
        :virtual-scroller-options="
          virtual ? { itemSize: 38, showLoader: true } : null
        "
        ...
        option-value="id"
        class="w-full"
        panel-class="cm-searchselect-panel"
        @change="emitChange"
      >
        ...
        <template #option="slotProps">
          <div class="flex flex-col leading-tight">
            ...
            <span v-if="slotProps.option.sublabel" class="text-xs text-gray-50">
    ...
    <small v-if="hint" class="mt-1 block text-xs text-gray-50">{{
      hint
    }}</small>
    <small v-if="errorText" class="mt-1 block text-xs text-danger">{{
      errorText
    }}</small>
  </div>
</template>
