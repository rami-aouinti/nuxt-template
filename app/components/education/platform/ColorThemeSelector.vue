...
import themeService from '~/services/colorThemeService.js'
...
async function loadThemes() {
  ...
  try {
    ...
    if (raw.length) {
      ...
      platformCurrentIri.value =
        raw.find((r) => r.active)?.colorTheme?.['@id'] || null
    }
  } catch (e) {
  ...
  if (!serverThemes.value.length) {
    try {
      ...
      const arr = Array.isArray(all?.items)
        ? all.items
        : Array.isArray(all)
          ? all
          : []
      ...
      platformCurrentIri.value =
        platformConfigStore.getSetting?.('platform.color_theme') || null
    } catch (e) {
