...
<template>
  <BaseCard>
    ...
    <div v-if="isLoading" class="space-y-2">
      <div v-for="v in 3" :key="v" class="flex flex-row gap-2 items-center">
        <Skeleton class="mr-auto" width="10rem" />
        <Skeleton size="2.5rem" />
        <Skeleton size="2.5rem" />
      </div>
    </div>

    <ul v-else-if="profileList.length > 0" class="space-y-2">
      <li
        v-for="(profile, i) in profileList"
        :key="i"
        class="flex flex-row gap-2 items-center"
      >
        <span class="mr-auto" v-text="profile.title" />
