...
const isUndefinedUrl = computed(() => {
  ...
  return (
    r.includes('undefined-url') || route.path.includes('/error/undefined-url')
  )
})
...
const menuItems = computed(() => {
  ...
  if (flags.value.faq)
    items.push({ label: t('FAQ'), url: router.resolve({ name: 'Faq' }).href })
  if (allowRegistration.value)
    items.push({ label: t('Registration'), url: '/main/auth/registration.php' })
  if (flags.value.demo)
    items.push({ label: t('Demo'), url: router.resolve({ name: 'Demo' }).href })
