...
export function useSidebarMenu() {
  ...
  const showTabs = computed(() => {
    const defaultTabs =
      platformConfigStore.getSetting('display.show_tabs') || []
    const tabsPerRoleJson =
      platformConfigStore.getSetting('display.show_tabs_per_role') || ''
  ...
  const rawShowCatalogue = platformConfigStore.getSetting(
  ...
  const isActive = (item) => {
    if (item.route) {
      return (
        ...
        (item.route.name && route.name === item.route.name)
      )
    } else if (item.items) {
  ...
  const menuItemMyCourse = computed(() => {
    ...
    if (
      ...
      showTabs.value.indexOf('my_courses') > -1
    ) {
      const courseItems = []
  ...
  const menuItemsAfterMyCourse = computed(() => {
    ...
    if (
      allowStudentCatalogue.value &&
      showTabs.value.indexOf('catalogue') > -1
    ) {
    ...
    if (showTabs.value.indexOf('reporting') > -1) {
      ...
      if (
        ...
        subItems.push({
          ...
          url:
            '/main/my_space/' +
            (securityStore.isHRM ? 'session.php' : 'index.php'),
        })
    ...
    if (
      showTabs.value.includes('session_admin') &&
      (securityStore.isAdmin || securityStore.isSessionAdmin)
    ) {
