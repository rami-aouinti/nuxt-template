...
type CreateOrderItemPayload = Record<string, unknown>
...
export default defineEventHandler(async (event) => {
  ...
  const body = await readBody<CreateOrderItemPayload>(event)
  ...
  return await broWorldEcommerceRequest<OrderResponse>(
    event,
    `/shop/orders/${encodeURIComponent(tokenValue)}/items`,
    {
      method: 'POST',
      body,
      headers,
